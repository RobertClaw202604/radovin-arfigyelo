...
function sorShopify(p) {
  const v = (p.variants && p.variants[0]) || {};
  return { nev: p.title || '', ar: parseFloat(v.price), url: 'https://' + (p.handle ? (p.handle + '') : '') };
}
...
async function teljesKatalogus(shop, cfg) {
  ...
  for (;;) {
    ...
    const formazott = tipus === 'shopify' ? sorok.map(sorShopify) : sorok.map(sorWoo);
...
async function katlistas(termek, shop, cfg) {
  try {
    ...
    if (!katalogus.length) return { ok: false, shop: shop.id, termek: termek.id, hiba: 'ures_katalogus', talalat: null };
    const tal = szigor(katalogus, termek);
    if (!tal) return { ok: false, shop: shop.id, termek: termek.id, hiba: 'nincs_pontos_talalat', talalat: null };
    return {
      ok: true,
      shop: shop.id,
      termek: termek.id,
    ...
    return { ok: false, shop: shop.id, termek: termek.id, hiba: 'katalógus_hiba:' + (e.message || '').slice(0, 80), talalat: null };
  }
}
