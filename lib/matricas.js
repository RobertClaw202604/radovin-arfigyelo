...
// Per-shop jóváhagyott tétel-aliasok helyettesítik a fajta-gátat (ShopRenter kétmenetes
// match, guide §6/Commit 3 „human approval record”). Ha `opts.tetel_aliasok` nem üres,
// akkor a jelöltnek a fajta helyett LEGALÁBB EGY jóváhagyott aliast pontosan kell
// tartalmaznia – a többi (márka, kiszerelés, évjárat, puttony) gát változatlan marad.
// Fontos: opts alapértelmezésben nincs, így a MEGLÉVŐ hívások (scraper, borhalo, tesztek)
// byte-azonosan viselkednek; ez a modul SOHA nem enyhíti a közös matchert.
function szigor(talalatok, termek, opts) {
  if (!talalatok || !talalatok.length) return null;
  const tetelAliasok = (opts && Array.isArray(opts.tetel_aliasok) && opts.tetel_aliasok.length)
    ? opts.tetel_aliasok
    : null;
  // Alias-módban a fajta-gát nem kötelező (a jóváhagyott aliasok pótolják), de a
  // kiszerelés HARD kapu lesz, hogy a 0,7l ne keveredjen az 1l-lel (never-false-price).
  const meretHardGatu = tetelAliasok != null;
  ...
  // Alias-módban (tetelAliasok jelenlétében) a fajta-gát LETILTVA – a shop által jóváhagyott
  // alias a hiteles útmutató, nem a belső fajta-szó.
  const fajtaNorm = (tetelAliasok == null && termek.fajta) ? norm(termek.fajta).replace(/\s/g, '') : '';
  ...
  for (const t of talalatok) {
    ...
    // Alias-vagy-fajta gát:
    //  - alias-mód: a jelölt nevének legalább egy jóváhagyott tétel-aliast pontosan kell
    //    tartalmaznia – különben érvénytelen (nem hamis ár). Szóköz-nélküli formán
    //    ellenőrizzük (u.a., ahogy a fajta is), így a „Double Cask” a
    //    „Macallan 12 Double Cask 0,7l”-ben illik.
    //  - alap-mód: a meglévő pontos fajta-gát változatlan.
    if (tetelAliasok) {
      const vanAlias = tetelAliasok.some((al) => {
        const a = norm(al).replace(/\s/g, '');
        return a.length > 0 && n.includes(a);
      });
      if (!vanAlias) continue;
    } else if (fajtaKotelezo && !n.includes(fajtaNorm)) {
    ...
    // Alias-módban a kiszerelés HARD kapu: a fajta-gát enyhítését a pontos méret
    // (ml/liter) kompenzálja, hogy sosem keverjünk 0,7l-es tételt az 1l-essel.
    if (meretHardGatu) {
      const tL = talalatLiter(nyers);
      if (kL == null || tL == null || kL <= 0 || Math.abs(kL - tL) >= 0.05) continue;
    }
