...
const { shoprenter } = require('./shoprenter.js');
const { opencart } = require('./opencart.js');
const { unas } = require('./unas.js');
...
async function htmlSimples(termek, shop, cfg) {
  ...
  // EGYSÉGESÍTÉS (matcher-v2): az összes kinyert jelölt (JSON-LD + szövegkörnyezet)
  // típusos candidate-ként megy át a szigorú matcher-v2-ün. CSAK teljes identitás-
  // egyezés (márka+tétel+méret+approved reference) ad árat – a legacy kulcsszó-
  // "legjobb egyezés" NEM dönthet (nem találhat ki márka-rokon árat).
  const jeloltek = [];
  for (const t of ldTermekek) {
    if (t.ar != null && t.nev) jeloltek.push({ nev: t.nev, ar: t.ar, url: t.url, forras: t.forras });
  }
  ...
  for (const blokk of blokkok) {
    ...
    const nev = nevMr ? nevMr[1].trim() : termek.nev;
    if (nev) jeloltek.push({ nev, ar, url: searchUrl, forras: 'kontextus' });
  }

  const candidates = jeloltek.map((t) => candidate({
    shopId: shop.id, shopProductId: t.url, name: t.nev, url: t.url,
    price: t.ar, currency: 'HUF', extractor: shop.adapter || 'html-simon', availability: 'in_stock',
  }));

  const donto = candidates.length
    ? selectExactCandidate(candidates, termek, shop.id)
    : { status: 'no_exact_match', selected: null };
  if (donto.status === 'matched') {
    return { ok: true, shop: shop.id, termek: termek.id, talalatok: jeloltek, talalat: { nev: donto.selected.name, ar: donto.selected.price, url: donto.selected.url } };
  }
  // needs_review / no_exact_match / mapping_drift / ambiguous_match → nincs ár (0 hamis).
  return { ok: true, shop: shop.id, termek: termek.id, talalatok: jeloltek, talalat: null };
}
    ...
    try {
      ...
      switch (shop.adapter) {
        ...
        case 'borhalo':
          ...
          break;
        case 'shoprenter':
          res = await shoprenter(termek, shop, cfg);
          break;
        case 'opencart':
          res = await opencart(termek, shop, cfg);
          break;
        case 'unas':
          res = await unas(termek, shop, cfg);
          break;
        case 'katlistas':
        case 'shopify':
