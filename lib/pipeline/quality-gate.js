...
function collectBaselineFailures(eredmenyek) {
  ...
  for (const item of eredmenyek) {
    ...
    const status = radovin && radovin.ar != null ? RESULT_STATUS.MATCHED : RESULT_STATUS.NO_EXACT_MATCH;
    if (status !== RESULT_STATUS.MATCHED) {
      baselineFailures.push({ termek_id: item.termek_id, status: status === RESULT_STATUS.MATCHED ? status : 'no_exact_match_or_failure' });
    } else {
      baselineMatched += 1;
    }
  }
