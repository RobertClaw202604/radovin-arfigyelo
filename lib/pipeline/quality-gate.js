 ...
 * @param {Object} options { baselineShopId, thresholds } – a thresholds a
 *        config/alerts.json validált értékei (guide §13: nem örök konstansok).
 */
function qualityGate(run, previousRun = [], options = {}) {
  ...
  const th = Object.assign(
    {
      coverage_regression_ratio: 0.75,
      large_price_change_min: 2.0,
      large_price_change_max: 0.5,
      extreme_price_change_min: 5.0,
      extreme_price_change_max: 0.2,
    },
    options.thresholds || {}
  );
  ...
  if (previousCompetitor > 0 && currentCompetitor < previousCompetitor * th.coverage_regression_ratio) {
    errors.push({
  ...
  for (const change of compareMatchedPrices(eredmenyek, regiek)) {
    if (change.ratio >= th.extreme_price_change_min || change.ratio <= th.extreme_price_change_max) {
      errors.push({ code: 'extreme_price_change_requires_review', ...change });
    } else if (change.ratio >= th.large_price_change_min || change.ratio <= th.large_price_change_max) {
