...
function evaluateCandidate(candidate, product, shopId) {
  ...
  if (expectedVintage != null) {
    ...
    if (!years.has(String(expectedVintage))) {
      // Emberileg jóváhagyott pontos referencia: ha a bolt NEM ír ki évjáratot a
      // névben (years üres), akkor NEM ellentmondás, hanem elhagyás → elfogadjuk
      // (a jóváhagyott referencia hitelesíti, hogy ugyanaz a bor). Ha a név
      // MÁS évjáratot ír (pl. 2019 várt 2024 helyett), az valódi ellentmondás → utasítjuk.
      if (years.size !== 0 || !approvedReferenceMatches(candidate, product, shopId)) {
  ...
  if (!Number.isFinite(expectedMl) || actualMl == null) {
    // Jóváhagyott pontos referencia: ha a bolt nem közli a kiszerelést (actualMl
    // ismeretlen), NEM mond ellent – az emberi ellenőrzés hitelesíti a párt. De ha
    // a név MÁS kiszerelést mond, az valódi volume_mismatch (lent), amit soha nem
    // hagyunk jóvá. (expectedMl hiánya viszont adat-hiba a mi oldalunkon → blokkol.)
    if (!Number.isFinite(expectedMl) || actualMl != null || !approvedReferenceMatches(candidate, product, shopId)) {
      return { accepted: false, code: 'unknown_volume', detail: { expected: expectedMl || null, actual: actualMl } };
    }
  } else if (Math.abs(expectedMl - actualMl) > 5) {
