...
async function borhalo(termek, shop, cfg) {
  try {
    ...
    // `selectExactCandidate` dönt, mely kizárólag a MINDEN kötelező kapun átjutott
    // jelöltet adja MATCHED-ként; minden más (no_exact_match / needs_review /
    // ambiguous_match / mapping_drift) NEM termel árat. Lásd lib/domain/matcher-v2.js.
    ...
    if (elbiralas.status !== 'matched' || !elbiralas.selected) {
      return { ok: false, shop: shop.id, termek: termek.id, hiba: elbiralas.status === 'needs_review' ? 'nincs_jovahagyott_referencia' : 'nincs_pontos_talalat', talalat: null, talalatok: katalogus, matcher: { status: elbiralas.status } };
    }
    ...
    return {
      ...
      shop: shop.id,
      termek: termek.id,
      talalat: { nev: tal.name || tal.nev, ar: tal.price, url: tal.url || shop.base_url, megjegyzes: 'via Borháló katalógus (GA4, strict matcher-v2)' },
      talalatok: katalogus,
    };
  } catch (e) {
...
module.exports = { borhalo, kategoriaTele };
