// Radovin árfigyelő – konfiguráció-validáció (Commit 1 / P0).
//
...
// §26 (config/shopok.json + config/termekek.json mezők), §11 (QA checklist).
...
// --- Termék identitás séma (a Commit 3 által hozzáadott azonosítási mezőkhöz) ---
const azonositasSchema = {
  ...
  properties: {
    ...
    marka_aliasok: { type: 'array', items: { type: 'string' }, minItems: 1 },
    gyarto: { type: 'string' },
...
function ellenorizTermekek(cfg) {
  ...
  for (const t of termekek) {
    ...
    // Aktív termék: az összes `azonositas` kötelező – csak akkor engedjük aktívként,
    // ha a Commit 3 már feltöltötte (vagy a legacy mezők megfelelők). Itt csak azt
    // garantáljuk, hogy az új `azonositas` BLOKK, ha jelen van, érvényes legyen.
...
module.exports = { ellenorizShopok, ellenorizTermekek, betoltEgesz };
