...
const STATUSZOK = ['active', 'pending', 'blocked', 'szuneteltetve'];
...
// Commit 8: riasztási küszöbök betöltése a config/alerts.json-ból (guide §13 – küszöbök
// validált konfigban, nem örök konstansok; §20 – a Telegram creds SOSE itt, csak env-ből).
function betoltAlerts() {
  const raw = JSON.parse(fs.readFileSync(path.join(CONFIG_DIR, 'alerts.json'), 'utf8'));
  // Feltérképezzük a kötelező mezőket; hiányzó/nem-szám esetén fail-fast.
  const req = ['coverage_regression_ratio', 'large_price_change_min', 'large_price_change_max', 'extreme_price_change_min', 'extreme_price_change_max', 'adapter_consecutive_failures', 'schedule_interval_min', 'schedule_grace_min'];
  for (const k of req) {
    const v = raw[k];
    if (typeof v !== 'number' || !Number.isFinite(v)) throw new Error(`Invalid config/alerts.json: "${k}" kötelező szám.`);
  }
  return raw;
}
